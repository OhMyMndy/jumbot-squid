const TICKET_STATUS_OPEN = 352781547798528
const TICKET_STATUS_CLOSED = 352786130075648


const APPLICATION_ID = process.env.APPLICATION_ID as string
const TICKETS_CATEGORY_ID = process.env.TICKETS_CATEGORY_ID as string
const ROLE_ADMIN_ID = process.env.ROLE_ADMIN_ID as string
const ROLE_HELPER_ID = process.env.ROLE_HELPER_ID as string
const GUILD_ID = process.env.GUILD_ID as string
const DISCORD_TOKEN = process.env.DISCORD_TOKEN as string


const INKLING_ROLE_ID = process.env.INKLING_ROLE_ID as string
const SQUID_ROLE_ID = process.env.SQUID_ROLE_ID as string
const ONLY_SQUIDS_ID = process.env.ONLY_SQUIDS_ID as string
const JUMBO_SQUID_ID = process.env.JUMBO_SQUID_ID as string


const STARBOARD_CHANNEL_ID = process.env.STARBOARD_CHANNEL_ID as string
const STARBOARD_LISTENING_CHANNEL_IDS = (process.env.STARBOARD_LISTENING_CHANNEL_IDS as string).split(",")
const STARBOARD_LISTENING_EMOJI_IDS = (process.env.STARBOARD_LISTENING_EMOJI_IDS as string).split(",")
export {
  TICKET_STATUS_CLOSED,
  TICKET_STATUS_OPEN,
  TICKETS_CATEGORY_ID,
  ROLE_ADMIN_ID,
  ROLE_HELPER_ID,
  GUILD_ID,
  DISCORD_TOKEN,
  APPLICATION_ID,
  INKLING_ROLE_ID,
  SQUID_ROLE_ID,
  ONLY_SQUIDS_ID,
  JUMBO_SQUID_ID,
  STARBOARD_CHANNEL_ID,
  STARBOARD_LISTENING_CHANNEL_IDS,
  STARBOARD_LISTENING_EMOJI_IDS
}
